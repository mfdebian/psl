...
function J() {
  if (F) return C;
  F = 1;
  const i = 2147483647, s = 36, c = 1, o = 26, t = 38, d = 700, z = 72, y = 128, g = "-", N = /^xn--/, R = /[^\0-\x7F]/, P = /[\x2E\u3002\uFF0E\uFF61]/g, V = {
  ...
  function S(a) {
    ...
    for (; m < n; ) {
      ...
      if (r >= 55296 && r <= 56319 && m < n) {
        ...
        (p & 64512) == 56320 ? e.push(((r & 1023) << 10) + (p & 1023) + 65536) : (e.push(r), m--);
      } else
        e.push(r);
    }
    return e;
  ...
  const W = (a) => String.fromCodePoint(...a), U = function(a) {
    ...
    let n = 0;
    for (a = m ? h(a / d) : a >> 1, a += h(a / e); a > _ * o >> 1; n += s)
      a = h(a / _);
    ...
    j < 0 && (j = 0);
    for (let u = 0; u < j; ++u)
      a.charCodeAt(u) >= 128 && f("not-basic"), e.push(a.charCodeAt(u));
    for (let u = j > 0 ? j + 1 : 0; u < m; ) {
      const l = n;
      for (let k = 1, w = s; ; w += s) {
        u >= m && f("invalid-input");
        const b = U(a.charCodeAt(u++));
        b >= s && f("invalid-input"), b > h((i - n) / k) && f("overflow"), n += b * k;
        const x = w <= p ? c : w >= p + o ? o : w - p;
        if (b < x)
          break;
        const q = s - x;
        k > h(i / q) && f("overflow"), k *= q;
      }
      const v = e.length + 1;
      p = T(n - l, v, l == 0), h(n / v) > i - r && f("overflow"), r += h(n / v), n %= v, e.splice(n++, 0, r);
    }
    ...
    for (j && e.push(g); u < m; ) {
      ...
      for (const k of a)
        k >= n && k < l && (l = k);
      const v = u + 1;
      l - n > h((i - r) / v) && f("overflow"), r += (l - n) * v, n = l;
      for (const k of a)
        if (k < n && ++r > i && f("overflow"), k === n) {
          let w = r;
          for (let b = s; ; b += s) {
          ...
          e.push(I(D(w, 0))), p = T(r, v, u === j), r = 0, ++u;
        }
      ++r, ++n;
    }
    return e.join("");
  ...
  return C = {
    ...
    toASCII: function(a) {
      return L(a, function(e) {
        return R.test(e) ? "xn--" + B(e) : e;
      });
    },
    toUnicode: function(a) {
      return L(a, function(e) {
        return N.test(e) ? E(e.slice(4).toLowerCase()) : e;
      });
    }
  }, C;
}
